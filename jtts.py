...
class MainWindow(pyglet.window.Window):
	...
	def on_draw(self):
		...
		for texid,objs in groupby(self.objects+self.monsters+self.bullets,lambda o:o.texid):
			...
			for obj in objs:
				...
				glTexCoord2f(0.0,0.0);glVertex3f(-size,z-size,0.0)
				glTexCoord2f(1.0,0.0);glVertex3f(+size,z-size,0.0)
				glTexCoord2f(1.0,1.0);glVertex3f(+size,z+size,0.0)
				glTexCoord2f(0.0,1.0);glVertex3f(-size,z+size,0.0)
				glEnd()
				glPopMatrix()
	...
	def draw2D(self):
		...
		for item in self.player.inventory:
			...
			c=560
			size=32
			glTexCoord2f(0.0,0.0);glVertex3f(c-size,y-size,0.0)
			glTexCoord2f(1.0,0.0);glVertex3f(c+size,y-size,0.0)
			glTexCoord2f(1.0,1.0);glVertex3f(c+size,y+size,0.0)
			glTexCoord2f(0.0,1.0);glVertex3f(c-size,y+size,0.0)
			glEnd()
